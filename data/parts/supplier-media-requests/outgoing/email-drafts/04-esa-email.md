# Email Draft 4: Exhaust Systems Australia

To: sales@exhaustsystemsaustralia.com.au

Phone: 03 8763 3099 / 07 2802 2351

Subject: Product image export request for Oak Flats Muffler Men catalogue - Exhaust Systems Australia

Attachment: `data\parts\supplier-media-requests\outgoing\2026-05-18-esa-media-request.zip`

Contact source: https://exhaustsystemsaustralia.com.au/pages/contact

## Body

```text
Hi Exhaust Systems Australia team,

We are preparing the Oak Flats Muffler Men online parts catalogue and need real product images for the attached rows.

Please start with the 2 P1 rows because they are launch-blocking.

Current request split:

- P1 launch-blocking rows: 2
- P2 image request rows: 9
- P3 exception-review rows: 0
- Total rows in this supplier request: 11

Image requirements:

- Send one clean product image per row where possible.
- Use transparent PNG where available. Clean product-only JPG, PNG or WebP files are also accepted.
- Avoid watermarks, lifestyle-only photos, logos, favicons and placeholder images.
- Minimum 900 px on the longest edge is preferred if available.
- Name files by SKU or Source UID so the import can match them automatically.

Best filenames:

- SKU.png
- SourceUID.png

If a product genuinely has no image available, please mark that row as no image available in the CSV and return it with the image export.

Requested files are included in the ZIP attachment:

- esa-p1.csv for P1 rows
- esa-all.csv for all missing-media rows
- esa-request.md for the full request notes

Thanks,
Oak Flats Muffler Men
```
